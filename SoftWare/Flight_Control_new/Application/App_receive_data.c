 ...
 *
 * 帧格式 (参考项目格式):
 * [0xAA][0xAA][0x03][LEN][DATA...][SUM]
 *   1B     1B     1B    1B   20B      1B
 * 注意：参考项目使用的是 0xAA 0xAA 作为帧头，不是 0xAA 0xAF
 *
 * @param buf 接收缓冲区
 * @param len 缓冲区长度
...
uint8_t ANO_DT_ParseRC(uint8_t *buf, uint8_t len, RC_Data_t *rc)
{
    ...
    if (data_len < 16)  // 至少需要 8 个通道 (16 字节)
        return 0;

    // 4. 计算总帧长度 = 4 字节头 + 数据区 + 1 字节校验和
    uint8_t frame_len = 4 + data_len + 1;
    if (frame_len > len || frame_len > 32)
        return 0;
    ...
    // 6. 解析通道数据 (大端模式：高字节在前，有符号数)
    // 参考项目代码：Remote.thr = (vs16)(*(data_buf+4)<<8)|*(data_buf+5)
    ...
    // 7. 限制范围在 1000-2000
    if (rc->thr < 1000) rc->thr = 1000;
    if (rc->thr > 2000) rc->thr = 2000;
    if (rc->yaw < 1000) rc->yaw = 1000;
    if (rc->yaw > 2000) rc->yaw = 2000;
    if (rc->rol < 1000) rc->rol = 1000;
    if (rc->rol > 2000) rc->rol = 2000;
    if (rc->pit < 1000) rc->pit = 1000;
    if (rc->pit > 2000) rc->pit = 2000;
    if (rc->aux1 < 1000) rc->aux1 = 1000;
    if (rc->aux1 > 2000) rc->aux1 = 2000;
    if (rc->aux2 < 1000) rc->aux2 = 1000;
    if (rc->aux2 > 2000) rc->aux2 = 2000;
    if (rc->aux3 < 1000) rc->aux3 = 1000;
    if (rc->aux3 > 2000) rc->aux3 = 2000;
    if (rc->aux4 < 1000) rc->aux4 = 1000;
    if (rc->aux4 > 2000) rc->aux4 = 2000;
