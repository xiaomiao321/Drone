...
{
    HAL_I2C_Mem_Write(&hi2c2, SPL06_ADDR_WRITE, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, 1000);
}
...
{
    HAL_I2C_Mem_Read(&hi2c2, SPL06_ADDR_READ, reg, I2C_MEMADD_SIZE_8BIT, data, size, 1000);
}
...
static void Int_SPL06_Read_Coef(void)
{
    ...
    spl06_coef.c00 = (int32_t)((int32_t)((buf[0] << 12) | (buf[1] << 4) | (buf[2] >> 4)));
    // 符号扩展 (20 位有符号数)
    if (spl06_coef.c00 & 0x80000)
        spl06_coef.c00 |= 0xFFF00000;
    
    // 读取 c10 (20 位) - 存储在 0x15 的低 4 位，0x16, 0x17
    // buf[2] 已经包含 0x15 的值 (低 4 位在 buf[2] & 0x0F)
    // 需要读取 0x16 和 0x17
    Int_SPL06_Read_Reg(0x16, buf, 2);  // buf[0]=0x16, buf[1]=0x17
    spl06_coef.c10 = (int32_t)((int32_t)(((buf[2] & 0x0F) << 16) | (buf[0] << 8) | buf[1]));
    // 符号扩展 (20 位有符号数)
 ...
 * @return 补偿后的压力 (Pa)
 */
static float Int_SPL06_Calc_Pressure(int32_t raw_pressure, int32_t raw_temperature)
{
    // 根据配置获取比例因子
    // 这里使用 64 次过采样的比例因子
    ...
    float pcomp = spl06_coef.c00 + 
                  Praw_sc * (spl06_coef.c10 + 
                  Praw_sc * (spl06_coef.c20 + 
                  Praw_sc * spl06_coef.c30)) +
                  Traw_sc * spl06_coef.c01 +
                  Traw_sc * Praw_sc * (spl06_coef.c11 + 
                  Praw_sc * spl06_coef.c21);
...
uint8_t Int_SPL06_Init(void)
{
    ...
    Int_SPL06_Write_Reg(SPL06_TMP_CFG, 0x80 | (SPL06_TMP_PRC << 4) | SPL06_TMP_RATE);
