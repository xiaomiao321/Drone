...
Motor_Struct left_top_motor = {
    .tim = &htim3, .channel = TIM_CHANNEL_1, .speed = 0};
Motor_Struct left_bottom_motor = {
    .tim = &htim4, .channel = TIM_CHANNEL_4, .speed = 0};
Motor_Struct right_top_motor = {
    .tim = &htim2, .channel = TIM_CHANNEL_2, .speed = 0};
Motor_Struct right_bottom_motor = {
    .tim = &htim1, .channel = TIM_CHANNEL_3, .speed = 0};
...
// 偏航角属于辅助功能 => 并不在平稳飞行的范围  只需要保证飞机不要在空中旋转即可
// => 明显的返回就行 偏航角PID结构体  => 可以只通过比例部分来实现
...
void App_flight_get_euler_angle(void) {
  ...
  // 打印加速度
  // debug_printf(":%d,%d,%d\n", gyro_accel_data.accel.accel_x,
  // gyro_accel_data.accel.accel_y, gyro_accel_data.accel.accel_z);
...
void App_flight_pid_process(void) {
  // 俯仰角
  // 1. 需要赋值目标值和测量值
  // 外环的目标角度 => 如果是平稳飞行 => 值为0 => 如果需要遥控飞行 =>
  // 目标角度就是遥控器的值 数值转换 => remote_data.pit(0-1000,500为中间点)
  // 控制范围在±10°
...
void App_flight_control_motor(void) {
  ...
  case IDLE:
    ...
    break;
  case NORMAL:
    // 俯仰角 => 向前飞有角速度 => 正误差 => 需要一个向后飞的反馈效果 =>
    // 前两个电机转的快 后两个转的慢 不同重要程度的PID控制结果
    // 可以进行适当的限制
  ...
  case FIX_HEIGHT:
    ...
    left_top_motor.speed =
        remote_data.thr + gyro_y_pid.output - gyro_x_pid.output +
        Com_limit(gyro_z_pid.output, 100, -100) + height_pid.output;
    left_bottom_motor.speed =
        remote_data.thr - gyro_y_pid.output - gyro_x_pid.output -
        Com_limit(gyro_z_pid.output, 100, -100) + height_pid.output;
    right_top_motor.speed =
        remote_data.thr + gyro_y_pid.output + gyro_x_pid.output -
        Com_limit(gyro_z_pid.output, 100, -100) + height_pid.output;
    right_bottom_motor.speed =
        remote_data.thr - gyro_y_pid.output + gyro_x_pid.output +
        Com_limit(gyro_z_pid.output, 100, -100) + height_pid.output;
    break;
  case FAIL:
