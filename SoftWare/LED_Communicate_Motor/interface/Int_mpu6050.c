...
{
    // HAL 库固有模式 I2C 写操作
    // 1. 句柄 (hi2c1) 2. 设备地址 (0x68) 3. 寄存器地址 reg 4. 寄存器地址位宽 5. 写数据的地址 6. 写字节个数 7. 超时时间
...
{
    // 1. 句柄 (hi2c1) 2. 设备地址 (0x68) 3. 寄存器地址 reg 4. 寄存器地址位宽 5. 读数据的地址 6. 读字节个数 7. 超时时间
...
{
    // 1. 等待飞机停止平稳
    // 判断飞机是否停止平稳的标准：前后两次加速度计的差值小于 200 或者 100
...
void Int_MPU6050_Init(void)
{
    ...
    // 5. 设置采样频率 => 采集的是加速度计和陀螺仪数据 => 默认频率 1000HZ => 1ms 读取一次
    // 采样频率应该高于后续数据的使用频率  否则丢失数据 => 奈奎斯特  采样频率 >= 2 倍使用频率
    // 设置采样分频为 2 => 即写入值为 2-1
